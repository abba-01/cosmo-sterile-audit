...
def update_provenance(repo_root, file_info, env_vars_used, actual_hash):
    ...
    provenance["files"][file_info['name']] = {
        ...
        "script": "scripts/03_fetch_gaia.py",
...
def fetch_gaia_data():
    """Download Gaia Cepheid parallax data and verify checksums."""
    ...
    gaia_config = manifest.get('gaia', {})
    files = gaia_config.get('files', [])

    if not files:
        print("ERROR: No files defined in manifest for 'gaia'", file=sys.stderr)
        return 1

    # Display query information if available
    if 'query' in gaia_config:
        print(f"\nExpected query: {gaia_config['query']}")
        print("NOTE: URL should point to pre-queried results matching the above query\n")
