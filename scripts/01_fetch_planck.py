...
def update_provenance(repo_root, file_info, env_vars_used, actual_hash):
    ...
    provenance["files"][file_info['name']] = {
        ...
        "script": "scripts/01_fetch_planck.py",
...
def fetch_planck_data():
    """Download Planck chains and verify checksums."""
    ...
    planck_config = manifest.get('planck', {})
    files = planck_config.get('files', [])

    if not files:
        print("ERROR: No files defined in manifest for 'planck'", file=sys.stderr)
        return 1
